...
q = ("SELECT unique_system_identifier, grant_date, location_class_code, "
...
for row in q.fetchall():
	uls_no, grant_date, class_code, lat, lon = row

	if not uls_no in license_locs:
		license_locs[uls_no] = [],[],grant_date
...
for uls_no, v in license_locs.items():
	transmitters, receivers, grant_date = v
	if len(transmitters) != 1 or len(receivers) < 1:
		print(uls_no, len(transmitters), 'transmitters', len(receivers), 'receivers')
		continue

	props = {'microwave_uls': uls_no, 'url': 'http://wireless2.fcc.gov/UlsApp/UlsSearch/license.jsp?licKey=%d' % uls_no, 'grant_date': grant_date}
	for r in receivers:
